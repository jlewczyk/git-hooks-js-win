...
module.exports = {
    ...
    install: function (workingDirectory) {
        ...
        if (!gitPath) {
            throw new Error('git-hooks-win must be run inside a git repository');
        }
        ...
        if (fsHelpers.exists(pathToGitHooks)) {
            throw new Error('git-hooks-win already installed');
        }

        if (fsHelpers.exists(hooksPath)) {
            fs.renameSync(hooksPath, pathToGitHooks);
        }
    ...
    uninstall: function (workingDirectory) {
        ...
        if (!fsHelpers.exists(hooksPath)) {
            throw new Error('git-hooks-win is not installed');
        }
